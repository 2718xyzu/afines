    ...
    if(!dist_sorted.empty()){
        ...
        for (set<pair<double, array<int, 2>>>::iterator it=dist_sorted.begin(); it!=dist_sorted.end(); ++it)
        {
           ...
           // else if(!(f_index[pr(hd)]==(it->second).at(0) && l_index[pr(hd)]==(it->second).at(1))) {
                ...
                not_off_prob += metropolis_prob(hd, it->second, intPoint, r_on*dt);
...
void motor::step_onehead(int hd)
{
    ...
    double off_prob = metropolis_prob(hd, {0,0}, hpos_new, at_barbed_end[hd] ? r_end*dt : r_off*dt);
    bool light_yes = 1;
    double radial = 20000;
...
string motor::to_string()
{
    ...
    sprintf(buffer, "\
            ...
            r_on*dt, r_off*dt, r_end*dt, dt, temperature, damp,
